...
$statsSql = "
    SELECT
        COUNT(*) AS total,
        SUM(CASE WHEN dr.status IN ('reported','assigned') THEN 1 ELSE 0 END) AS open,
        SUM(CASE WHEN dr.status = 'in_progress' THEN 1 ELSE 0 END) AS in_progress,
        SUM(CASE WHEN dr.status IN ('completed','verified','closed') THEN 1 ELSE 0 END) AS resolved,
        SUM(CASE WHEN dr.priority = 'critical' THEN 1 ELSE 0 END) AS critical
    FROM defect_reports dr
";
$statsScope = [];
if (isset($defectReportColumns['is_public'])) {
    $statsScope[] = "dr.is_public = true";
} elseif (isset($defectReportColumns['admin_approval_status'])) {
...
body{font-family:'DM Sans',sans-serif;background:var(--p);min-height:100vh;padding:0;position:relative;overflow-x:hidden;}
...
/* (top navigation now provided by the shared includes/site_nav.php) */
