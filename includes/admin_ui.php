<?php
/**
 * includes/admin_ui.php — admin shell behaviour. Replaces site_transitions.php
 * on admin pages only; the public / reporter surfaces keep the branded
 * full-screen loader, because there the wait is the login round trip. Admin is
 * a work surface, so a curtain drop on every click is just added latency.
 *
 * Provides:
 *   - a 2px top progress bar for navigations and POSTs, with no artificial delay
 ...
 *   - a sidebar whose scroll position survives a page load
 *   - stops the decorative infinite rotations inherited from the page CSS
 *
 * Include once before </body>.
 *
 * NOTE: `html{font-size:106.25%}` deliberately does NOT live here. It sits in
 * each admin page's <head> style block instead — setting the root font size
 * from an end-of-<body> include relays out the entire page after first paint,
 * which is what the "reload / glitching" effect on admin pages actually was.
 */
...
/* ---- logout confirmation ------------------------------------------------- */
/* The dashboard used to be the only page that confirmed before signing out.
   Promoting it here gives all 11 admin pages the same behaviour from one place. */
#becLogout{position:fixed;inset:0;z-index:100001;display:none;
  align-items:center;justify-content:center;padding:1rem;
  background:rgba(26,8,8,.55);}
#becLogout.on{display:flex;}
#becLogout .bl-card{background:#fff;border:1px solid #E2D9CC;border-radius:16px;
...
#becLogout h3{font-family:'Outfit',sans-serif;font-size:1.05rem;font-weight:800;
  color:#1C1008;margin:0 0 .3rem;}
#becLogout p{font-size:.82rem;color:#755B4E;margin:0 0 1.15rem;line-height:1.45;}
...
#becLogout button,#becLogout a{font-family:'DM Sans',sans-serif;font-size:.82rem;
  ...
  text-decoration:none;border:1px solid #E2D9CC;}
#becLogout .bl-no{background:#FBF9F6;color:#5C3838;}
...
@media (prefers-reduced-motion:reduce){
  ...
  .mo.open,.mo .mw,#becLogout .bl-card{animation:none;}
}
</style>
...
(function () {
  ...
  // Link navigations: start the bar and let the browser navigate normally. The
  // old include did preventDefault + setTimeout(140ms) so its overlay had time
  // to paint — that delay was pure, self-inflicted click latency.
  ...
  // POSTs: progress bar only. Locking the submit buttons against a slow Supabase
  // round trip is already owned by includes/admin_assistant.php (which is on all
  // 12 admin pages) — it calls becShowLoader() above, so this is just the fallback
  // for any GET form that submits without going through that handler.
